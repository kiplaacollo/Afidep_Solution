Report 50084 "Staff Cost Report Per employee"
{
    RDLCLayout = 'Layouts/StaffCostR.rdlc';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("Payroll Project Allocation"; "Payroll Project Allocation")
        {
            ...
            begin
                ...
                WorkingDays := GetWorkingDays(Period);
    ...
    begin
        ...
        TotalCoreHours := AD003Hours + RM001Hours + RM2025Hours;
        TotalEmployeeHours := TotalCoreHours + OtherHours;
        TargetHours := WorkingDays * 8; // fixed standard for the month

        // Step 2: Apply adjustment logic
        if TotalEmployeeHours <= TargetHours then begin
            // No adjustment needed
            AdjustedHours := LineHours;
        end else begin
            if (OtherHours < TargetHours) then begin
                // Non-core projects under 176, distribute remaining to AD/RM
                ExtraHours := TargetHours - OtherHours;

                if (ProjCode in ['AD03', 'RM001', 'RM2025']) and (TotalCoreHours > 0) then
                    AdjustedHours := ExtraHours * (LineHours / TotalCoreHours)
                else
                    AdjustedHours := LineHours; // Keep actuals for non-core
            end else begin
                // Non-core projects exceed 176 — scale them down proportionally
                if not (ProjCode in ['AD03', 'RM001', 'RM2025']) and (OtherHours > 0) then
                    AdjustedHours := LineHours * (TargetHours / OtherHours)
                else
                    AdjustedHours := 0; // Core projects get zero in this case
            end;
        end;
        ...
        // Step 3: Return proportional amount based on adjusted hours
        exit((AdjustedHours / 8) * (TotalCost / WorkingDays)); // use 22 days for rate
    end;
