Codeunit 80013 "Payroll Management_Malawi"
{
    ...
    var
        ...
        EmployeeTransactions: Record "Payroll Employee Trans_Malawi";
        ...
        NSSFTOUSE: Decimal;
        CURRTOUSE: Decimal;
        PayrollMonthly: Record "Payroll Employee Trans_Malawi";
        PrEmployee2: Record "Payroll Employee_AU";
        NSSFtoUSE2: Decimal;
        PayrollMonthlyTrans_AU: Record "Payroll Monthly Trans_Malawi";
    ...
    begin
        ...
        ProcessBasicPay("Employee No", "Payroll Period", "Posting Group", BasicPay, "BasicPay(LCY)", "Currency Code", "Currency Factor", "Joining Date", "Leaving Date",
                        BasedOnTimeSheet, "Global Dimension 1 Code", "Global Dimension 2 Code", Department, "Pension %-Employer", "Pension Administrative Fee", "VAT Administrative Fee", "Group Life Assuarance");
        ...
        END;
        if "Global Dimension 1 Code" = 'MALAWI' then begin
    ...
    begin
        ...
        Initialize("Payroll Period");
        //Basic Pay
        // ProcessBasicPay("Employee No", "Payroll Period", "Posting Group", BasicPay, "BasicPay(LCY)", "Currency Code", "Currency Factor",
        //  "Joining Date", "Leaving Date", BasedOnTimeSheet, "Global Dimension 1 Code", "Global Dimension 2 Code", Department,
        //   "Pension %-Employer", "Pension Administrative Fee", "VAT Administrative Fee", "Group Life Assuarance");
    ...
    local procedure ProcessBasicPay("Employee No": Code[20]; "Payroll Period": Date; "Posting Group": Code[20]; BasicPay: Decimal; "BasicPay(LCY)": Decimal; "Currency Code": Code[20]; "Currency Factor": Decimal; "Joining Date": Date; "Leaving Date": Date; BasedOnTimeSheet: Boolean; "Global Dimension 1 Code": Code[20]; "Global Dimension 2 Code": Code[20]; Department: Code[20]; "Pension %-Employer": Decimal; "Pension Administrative Fee": Decimal; "VAT Administrative Fee": Decimal; "Group Life Assuarance": Decimal)
    var
        DaysInMonth: Integer;
        DaysWorked: Integer;
        ...
        VATFee: Decimal;
        Grouplife: Decimal;
        "EmployerPension(LCY)": Decimal;
        "AdmPension(LCY)": Decimal;
        "VATFee(LCY)": Decimal;
        "Grouplife(LCY)": Decimal;
    begin
        //Setup Constants
        ...
        "Transaction Code" := 'Pension_Employer';
        "Transaction Type" := 'Pension';
        TransDescription := 'Employers Pension';
        Grouping := 1;
        SubGrouping := 2;
        EmployerPension := currentAmount * "Pension %-Employer" / 100;
        "EmployerPension(LCY)" := "currentAmount(LCY)" * "Pension %-Employer" / 100;
        InsertMonthlyTransactions("Employee No", "Transaction Code", "Transaction Type", Grouping, SubGrouping,
       TransDescription, EmployerPension, "EmployerPension(LCY)", 0, 0, "Payroll Period", CurMonth, CurYear, "Account Type",
       "Account No.", "posting type"::Debit, "Global Dimension 1 Code", "Global Dimension 2 Code", EmployeeTransactions.Membership,
       EmployeeTransactions."Reference No", Department, "co-op", LoanNo);

        "Transaction Code" := 'Adm-Pension';
        "Transaction Type" := 'Adm_Pension';
        TransDescription := 'Administrative Pension fee';
        Grouping := 1;
        SubGrouping := 3;
        AdmPension := currentAmount * "Pension Administrative Fee" / 100;
        "AdmPension(LCY)" := "currentAmount(LCY)" * "Pension Administrative Fee" / 100;
        InsertMonthlyTransactions("Employee No", "Transaction Code", "Transaction Type", Grouping, SubGrouping,
       TransDescription, AdmPension, "AdmPension(LCY)", 0, 0, "Payroll Period", CurMonth, CurYear, "Account Type",
       "Account No.", "posting type"::Debit, "Global Dimension 1 Code", "Global Dimension 2 Code", EmployeeTransactions.Membership,
       EmployeeTransactions."Reference No", Department, "co-op", LoanNo);

        "Transaction Code" := 'Vat-Adm-Pension';
        "Transaction Type" := 'Vat_Adm_Pension';
        TransDescription := 'Vat Administrative Pension fee';
        Grouping := 1;
        SubGrouping := 4;
        VATFee := AdmPension * "VAT Administrative Fee" / 100;
        "VATFee(LCY)" := "AdmPension(LCY)" * "VAT Administrative Fee" / 100;
        InsertMonthlyTransactions("Employee No", "Transaction Code", "Transaction Type", Grouping, SubGrouping,
       TransDescription, VATFee, "VATFee(LCY)", 0, 0, "Payroll Period", CurMonth, CurYear, "Account Type",
       "Account No.", "posting type"::Debit, "Global Dimension 1 Code", "Global Dimension 2 Code", EmployeeTransactions.Membership,
       EmployeeTransactions."Reference No", Department, "co-op", LoanNo);

        "Transaction Code" := 'GroupLife';
        "Transaction Type" := 'GroupLife';
        TransDescription := 'Group Life';
        Grouping := 1;
        SubGrouping := 5;
        Grouplife := currentAmount * "Group Life Assuarance" / 100;
        "Grouplife(LCY)" := "currentAmount(LCY)" * "Group Life Assuarance" / 100;
        InsertMonthlyTransactions("Employee No", "Transaction Code", "Transaction Type", Grouping, SubGrouping,
       TransDescription, Grouplife, "Grouplife(LCY)", 0, 0, "Payroll Period", CurMonth, CurYear, "Account Type",
       "Account No.", "posting type"::Debit, "Global Dimension 1 Code", "Global Dimension 2 Code", EmployeeTransactions.Membership,
       EmployeeTransactions."Reference No", Department, "co-op", LoanNo);
    end;
    ...
    begin
        ...
        IF PrEmployee.FIND('-') THEN BEGIN
            ...
            PensionBenefit := Round(PrEmployee."Basic Pay" * PrEmployee."Pension %-Employee" / 100, 0.01, '=');//+ EmpNSSF;
    ...
    begin
        ...
        IF PrEmployee.FIND('-') THEN BEGIN
            ...
            Error('pension Admin %1, Vat pens %2, Basic Pay %3', PensionAdministrativeFee, VatOnPension, PrEmployee."Basic Pay");
    ...
    local procedure ProcessVatPension("Employee No": Code[20]; "Payroll Period": Date; "Posting Group": Code[20]; BasicPay: Decimal; "BasicPay(LCY)": Decimal; "Currency Code": Code[20]; "Currency Factor": Decimal; "Joining Date": Date; "Leaving Date": Date; BasedOnTimeSheet: Boolean; "Global Dimension 1 Code": Code[20]; "Global Dimension 2 Code": Code[20]; Department: Code[20]; PayesNSSF: Boolean)
    var
        NSSFAmt: Decimal;
        "NSSFAmt(LCY)": Decimal;
        PayrollEmployee: Record "Payroll Employee_AU";
    begin
        ...
        IF PrEmployee.FIND('-') THEN BEGIN
            ...
            VatOnPension := Round(PensionAdministrativeFee * PrEmployee."VAT Administrative Fee" / 100, 0.01, '=');
            Error('pension Admin %1, Vat pens %2, Basic Pay %3', PensionAdministrativeFee, VatOnPension, PrEmployee."Basic Pay");
            "Transaction Code" := 'VAT ON PENSION';
            TransDescription := 'VAT ON PENSION';
            "Transaction Type" := 'TAX CALCULATIONS';
    ...
    begin
        ...
        if curPensionStaff > MaxPensionContrib then
            EmpTaxableEarning := EmpGrossTaxable - (EmpSalaryArrear + EmpDefinedContrib + EmpOOI + EmpHOSP + EmpNonTaxable)//+ MaxPensionContrib 

        else
            //ERROR(FORMAT(EmpGrossTaxable));
        EmpTaxableEarning := EmpGrossTaxable - (EmpSalaryArrear + EmpDefinedContrib + curPensionStaff + EmpOOI + EmpHOSP + EmpNonTaxable);
        ...
        if PrEmployee."Global Dimension 1" = 'KENYA' THEN begin
            ...
            if PensionBenefit > MaxPensionContrib then begin
                EmpTaxableEarning := Round(EmpTaxableEarning - (MaxPensionContrib), 0.01, '=')//+ EmpNSSF
            end else
                EmpTaxableEarning := Round(EmpTaxableEarning - (PensionBenefit + EmpNSSF), 0.01, '=');
        END;
    ...
    begin
        ...
        currentAmount := Round(EmpPaye, 0.01, '=');
        "currentAmount(LCY)" := Round("EmpPaye(LCY)" - EmpPersonalRelief - InsuranceAmount, 0.01, '=');
    ...
    var
        MonthlyTransactions: Record "Payroll Monthly Trans_Malawi";
        PayrollEmployee: Record "Payroll Employee_AU";
    begin

        if "currentAmount(LCY)" = 0 then exit;
    ...
    var
        PayrollEmpTrans: Record "Payroll Employee Trans_Malawi";
    begin
    ...
    var
        ...
        NewYear: Integer;
        MonthlyTransactions: Record "Payroll Monthly Trans_Malawi";
        EmployeeTrans: Record "Payroll Employee Trans_Malawi";
        TCODE: label 'NEGP';
    ...
    var
        ...
        NewYear: Integer;
        NewMonth: Integer;
        EmployeeTransactions2: Record "Payroll Employee Trans_Malawi";
        PayrollCalender: Record "Payroll CalenderMalawi";
        NewPayrollCalender: Record "Payroll CalenderMalawi";
    ...
    begin
        ...
        if PayrollCalender.FindFirst then begin
            ...
            PayrollCalender."Branch Code" := 'Malawi';
