Page 80122 "Purchase Invoice List Appr"
{
    Caption = 'Approved Purchase Invoice List';
    CardPageID = 51;
    // InsertAllowed = false;
    Editable = false;
    PageType = List;
    ...
    SourceTableView = sorting("Document Type", "No.")
                      order(descending)
                      where(/*pv = const(true),*/ Status = filter(Released), "Document Type" = filter('Invoice'));// "Document Type" = filter('Payment Voucher'));
