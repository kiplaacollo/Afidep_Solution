Page 80121 "Purchase Invoice List Pend"
{
    Caption = 'Pending Purchase Invoice List';
    CardPageID = 51;
    // InsertAllowed = false;
    Editable = false;
    PageType = List;
    ...
    SourceTableView = sorting("Document Type", "No.")
                      order(descending)
                      where(Status = filter("Pending Approval"), "Document Type" = filter('Invoice'));// "Document Type" = filter('Payment Voucher'));
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Document Total"; Rec."Document Total") { ApplicationArea = all; Caption = 'Amount in Foreign'; }
                ...
                field("Consultancy Tax Amount"; Rec."Consultancy Tax Amount")
                {
                    ApplicationArea = all;
                    Visible = false;
                }
