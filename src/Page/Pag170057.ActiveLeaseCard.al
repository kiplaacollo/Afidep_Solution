Page 170057 "Active Lease Card"
{
    ...
    actions
    {
        area(navigation)
        {
            ...
            {
                Caption = 'Lease';
                action("Mark Lease As Inactive")
                {
                    ApplicationArea = Basic;
                    Caption = 'Mark Lease As Inactive';
                    Image = "Action";
                    Promoted = true;
                    ...
                    trigger OnAction()
                    begin
                        if Confirm('Are you sure you want deactivate lease?', true, false) = true then begin
                            Units.Reset;
                            Units.SetRange(Units."No.", Rec.Unit);
                            if Units.FindFirst then begin
                                Units.Tenant22 := '';
                                Units."Tenant Full Name 33" := '';
                                Units."Unit Status3" := Units."unit status3"::Vacant;
                                Units.Modify;
                            end;
                            Rec.Posted := true;
                            Rec."Posted Date" := Today;
                            Rec."Posted By" := UserId;
                            Rec."Lease Status" := Rec."lease status"::Inactive;
                            Rec.Modify;
                        end;
                    end;
                }
